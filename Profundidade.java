...
class Profundidade {
	...
	public static void dfs(Stack<nodes> stack, int[] inicial, int[] goal) {
		...
		while (!stack.empty()) {
			...
			gerados=cria_filhos_stack(stack, atual, visitados, gerados);
			if(atual.nfilhos==0){ visitados.remove(Arrays.toString(atual.vetor));
								  atual.pai.nfilhos--;}
	...
	public static int cria_filhos_stack(Stack<nodes> stack, nodes atual, ArrayList<String> visitados, int gerados) {
       	if(atual.profundidade>20) return gerados;
		int blankpos=-1;
